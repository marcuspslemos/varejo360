...
rows = []
for d in dates:
    saz = 1.0 + 0.15 * np.sin(2 * np.pi * (d.timetuple().tm_yday) / 365)  # sazonalidade anual
    for loja in lojas:
        for canal in canais:
            sample_skus = np.random.choice(skus, size=25, replace=False)
            for sku in sample_skus:
                base_q = np.random.poisson(5)
                promo = np.random.rand() < 0.12  # 12% em promoção
                ...
                cogs = qtd * cogs_unit
                margem = vendas - cogs
                estoque_custo = np.random.uniform(50, 400)
                otif_ok = np.random.rand() < 0.93
                transacoes = max(1, int(np.round(qtd / np.random.uniform(1.2, 2.8))))
                rows.append([
                    d, loja, canal, sku, vendas, qtd, margem, cogs,
                    estoque_custo, promo, otif_ok, transacoes
                ])

df = pd.DataFrame(rows, columns=[
    "data", "loja", "canal", "sku", "vendas", "qtd", "margem", "cogs",
    "estoque_custo", "promo", "otif_ok", "transacoes"
...
# -----------------------------
# 6) Campo de PREVISIBILIDADE (Forecast)
# -----------------------------
...
if len(serie_cat.dropna()) > 20:
    ...
    if back_n > 0:
        ...
        model_bt = ExponentialSmoothing(
            hist_train, trend="add", seasonal="add", seasonal_periods=52, initialization_method="estimated"
        ).fit()
